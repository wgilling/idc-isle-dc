...
docker-compose exec drupal bash -lc "set -vex ; drush en jwt_auth_issuer && apk add jq && printf '%s-' $(date '+%s') > /tmp/calc ; curl -s -u admin:password http://localhost/jwt/token | jq .token |cut -f 2 -d "."|base64 -d 2>/dev/null|awk 'BEGIN {RS=\",\";FS=\":\";} /exp/ { print \$2\"\\n\"}' | sed -e '/^$/d' >> /tmp/calc ; cat /tmp/calc; bc < /tmp/calc |sed -e s/-// > /tmp/res ; [ 14400 -le \`cat /tmp/res\` ]"
